...
func (r *clinicRepo) Create(ctx context.Context, clinic *clinic.Clinic) error {
	query := `INSERT INTO tbl_clinic (id, user_id, name, abn_number, address, city, state, postcode, phone, email, website, logo_url, description, share_type, clinic_share, owner_share, method_type, is_active, with_holding_tax, created_at, updated_at)
		VALUES (:id, :user_id, :name, :abn_number, :address, :city, :state, :postcode, :phone, :email, :website, :logo_url, :description, :share_type, :clinic_share, :owner_share, :method_type, :is_active, :with_holding_tax, :created_at, :updated_at)`
	_, err := r.db.NamedExecContext(ctx, query, clinic)
	return err
}

func (r *clinicRepo) GetByID(ctx context.Context, id string) (*clinic.Clinic, error) {
	query := `SELECT id, user_id, name, abn_number, address, city, state, postcode, phone, email, website, logo_url, description, share_type, clinic_share, owner_share, method_type, is_active, with_holding_tax, created_at, updated_at FROM tbl_clinic WHERE id = $1 AND deleted_at IS NULL`
...
func (r *clinicRepo) SetActive(ctx context.Context, id string, active bool) error {
	query := `UPDATE tbl_clinic SET is_active = $1, updated_at = $2 WHERE id = $3 AND deleted_at IS NULL`
	res, err := r.db.ExecContext(ctx, query, active, time.Now(), id)
	if err != nil {
		return err
	}
...
func (r *clinicRepo) List(ctx context.Context) ([]clinic.Clinic, error) {
	query := `SELECT id, user_id, name, abn_number, address, city, state, postcode, phone, email, website, logo_url, description, share_type, clinic_share, owner_share, method_type, is_active, with_holding_tax, created_at, updated_at FROM tbl_clinic WHERE deleted_at IS NULL`
...
func (r *clinicRepo) GetByABN(ctx context.Context, abnNumber string) (*clinic.Clinic, error) {
	query := `SELECT id, user_id, name, abn_number, address, city, state, postcode, phone, email, website, logo_url, description, share_type, clinic_share, owner_share, method_type, is_active, with_holding_tax, created_at, updated_at FROM tbl_clinic WHERE abn_number = $1 AND deleted_at IS NULL`
	var clinic clinic.Clinic
