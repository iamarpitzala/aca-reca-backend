...
func (f *financialQuarterRepo) Delete(ctx context.Context, id string) error {
...
func (f *financialQuarterRepo) GetByFinancialYearID(ctx context.Context, financialYearID int) (*clinic.FinancialQuarter, error) {
	query := `SELECT id, financial_year_id, name, quarter_number, start_date, end_date, is_closed, closed_at, closed_by, created_at, updated_at, deleted_at FROM tbl_financial_quarter WHERE financial_year_id = $1 AND deleted_at IS NULL`
...
func (f *financialQuarterRepo) GetByID(ctx context.Context, id string) (*clinic.FinancialQuarter, error) {
