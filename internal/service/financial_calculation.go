...
func (fcs *FinancialCalculationService) calculateGrossMethod(config map[string]interface{}, input domain.CalculationInput) (*domain.CalculationResult, *domain.BASMapping, error) {
	...
	// Set defaults
	if grossConfig.ServiceFeePercent == 0 {
		grossConfig.ServiceFeePercent = 60.0
	}
	if grossConfig.GSTOnServiceFeePercent == 0 {
		grossConfig.GSTOnServiceFeePercent = 10.0
	}
	if grossConfig.LabFeePaidBy == "" {
		grossConfig.LabFeePaidBy = "clinic"
	}
	if grossConfig.OutworkRatePercent == 0 {
		grossConfig.OutworkRatePercent = 40.0
	}
	...
	} else if grossConfig.GSTOnLabFee {
